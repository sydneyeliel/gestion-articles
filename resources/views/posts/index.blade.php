...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    {{-- ── HEADING ─────────────────────────────────────── --}}
    <h1 class="font-bold mb-8" style="font-size:2.75rem;color:#1A1A2E;line-height:1.15;letter-spacing:-0.5px;">
        Latest Articles
    </h1>
    ...
    @if($posts->count() && !request('search') && !request('category') && $posts->currentPage() === 1)
        ...
        <div class="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-300 mb-12"
             style="display:grid;grid-template-columns:60% 40%;">

            {{-- Image (left) --}}
            <a href="{{ route('posts.show', $featured->slug) }}"
               class="block overflow-hidden bg-gray-900"
               style="min-height:340px;">
                ...
                @else
                    <div class="w-full h-full flex items-center justify-center"
                         style="min-height:340px;background:#0f0f1a;">
                        <svg class="w-16 h-16 opacity-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-8 lg:p-10 flex flex-col justify-center">
                ...
                <h2 class="font-bold leading-snug mb-4" style="font-size:1.65rem;color:#1A1A2E;">
                    <a href="{{ route('posts.show', $featured->slug) }}"
                ...
                <div class="flex items-center gap-3 mt-auto">
                    <div class="w-9 h-9 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center text-white text-sm font-bold"
                         style="background:#4A4A8A;">
                        {{ strtoupper(substr($featured->user->name, 0, 1)) }}
                    </div>
                    <div>
                        <p class="text-sm font-semibold" style="color:#1A1A2E;">
                            {{ $featured->user->name }}
                        </p>
    ...
    <form action="{{ route('home') }}" method="GET" class="sm:hidden mb-6">
        ...
        <div class="relative">
            ...
            <input type="search" name="search" value="{{ request('search') }}"
                   ...
                   class="w-full pl-10 pr-4 py-3 rounded-full border border-gray-200 bg-gray-50 text-sm focus:outline-none focus:ring-2"
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <section class="flex-1 min-w-0">
            ...
            @if(request('search') || request('category'))
                <div class="flex flex-wrap items-center gap-2 mb-6 text-sm text-gray-500">
                    ...
                    @endif
                    <a href="{{ route('home') }}" class="text-xs text-red-400 hover:text-red-600 transition-colors">
                        ✕ Clear
                    </a>
                </div>
            @endif
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                    @foreach($posts as $post)
                        <article class="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 flex flex-col group">
                            ...
                            <div class="relative overflow-hidden flex-shrink-0 bg-gray-100"
                                ...
                                <a href="{{ route('posts.show', $post->slug) }}" class="block w-full h-full">
                                    ...
                                    @else
                                        <div class="w-full h-full" style="background:#0f0f1a;"></div>
                                    @endif
                                </a>
                                <a href="{{ route('home', ['category' => $post->category->slug]) }}"
                                   ...
                                   style="background:rgba(15,15,26,0.72);backdrop-filter:blur(4px);">
                                    {{ $post->category->name }}
                                </a>
                            </div>
                            ...
                            <div class="p-5 flex flex-col flex-1">
                                ...
                                {{-- Excerpt --}}
                                <p class="text-sm leading-relaxed line-clamp-3 flex-1 mb-4" style="color:#6B7280;">
                                    {{ $post->excerpt }}
                                </p>
                                ...
                                <div class="flex items-center justify-between gap-3 pt-3 border-t border-gray-100">
                                    <div class="flex items-center gap-2 min-w-0">
                                        ...
                                        <span class="text-xs font-medium truncate" style="color:#1A1A2E;">
                                            {{ $post->user->name }}
                                        </span>
                                    </div>
                                    <a href="{{ route('posts.show', $post->slug) }}"
                                       class="flex items-center gap-1 text-xs font-semibold whitespace-nowrap flex-shrink-0 transition-all hover:gap-2"
                                       style="color:#6C63FF;">
                                        Read more
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <div class="text-center py-24">
                    ...
                    <a href="{{ route('home') }}"
                       class="inline-block text-sm font-semibold px-5 py-2.5 rounded-xl transition-all hover:opacity-90"
        ...
        {{-- ── SIDEBAR ─────────────────────────────────── --}}
        <aside id="categories" class="lg:w-64 xl:w-72 flex-shrink-0 space-y-5">

            {{-- Categories --}}
            <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                <h3 class="font-bold text-sm mb-4" style="color:#1A1A2E;letter-spacing:0.01em;">Categories</h3>
                <ul class="space-y-3">
                    @foreach($categories as $cat)
                        <li>
                            <a href="{{ route('home', array_filter(['category' => $cat->slug, 'search' => request('search')])) }}"
                               class="flex items-center justify-between group py-0.5">
                                ...
                                <span class="text-sm font-medium" style="color:#9CA3AF;">
                                    {{ $cat->posts_count }}
                                </span>
                            </a>
                        </li>
            ...
            {{-- Recent Posts --}}
            <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                <h3 class="font-bold text-sm mb-4" style="color:#1A1A2E;letter-spacing:0.01em;">Recent Posts</h3>
                <ul class="space-y-4">
                    @foreach($recent as $r)
            ...
            <div class="rounded-2xl p-6" style="background:#4A4A8A;">
                ...
                <p class="text-sm mb-5" style="color:rgba(255,255,255,0.7);line-height:1.5;">
                    Join 15,000+ readers who receive our weekly curation of thoughtful articles.
                </p>
                <form onsubmit="return false;">
                    <input type="email"
                           placeholder="Your email address"
                           class="w-full px-4 py-2.5 rounded-xl text-sm mb-3 focus:outline-none focus:ring-2 transition-all"
                    ...
                    <button type="submit"
                            class="w-full py-2.5 rounded-xl text-sm font-semibold text-white transition-all hover:opacity-90 active:scale-95"
...
<section id="newsletter" class="mt-20 scroll-mt-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="rounded-3xl px-8 py-16 md:px-16 text-center relative overflow-hidden"
         style="background:linear-gradient(135deg,#1A1A2E 0%,#4A4A8A 60%,#6C63FF 100%);">

        <div class="absolute top-0 right-0 w-72 h-72 rounded-full opacity-10 pointer-events-none"
             style="background:#6C63FF;transform:translate(35%,-35%);"></div>
        <div class="absolute bottom-0 left-0 w-56 h-56 rounded-full opacity-10 pointer-events-none"
             style="background:#6C63FF;transform:translate(-35%,35%);"></div>

        <div class="relative max-w-xl mx-auto">
            <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold mb-5 text-white"
            ...
            <form onsubmit="return false;" class="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
                <input type="email"
                       placeholder="Your email address"
                       class="flex-1 px-5 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 text-white"
                       style="background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.2);"
                       onfocus="this.style.background='rgba(255,255,255,0.18)'"
                       onblur="this.style.background='rgba(255,255,255,0.12)'">
                <button type="submit"
                        class="px-6 py-3 rounded-xl text-sm font-semibold text-white whitespace-nowrap transition-all hover:opacity-90 active:scale-95"
                        style="background:#6C63FF;">
                    Subscribe free →
                </button>
            </form>
    ...
    <div class="grid md:grid-cols-2 gap-12 items-center">
        ...
        <div>
            ...
            <p class="leading-relaxed mb-4" style="color:#6B7280;">
                NeyDys is an independent publication dedicated to ideas that matter. We believe quality writing deserves a clean, distraction-free home — no clutter, just content.
            </p>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm text-center">
                ...
                <p class="text-sm font-medium" style="color:#6B7280;">Articles published</p>
            </div>
            <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm text-center">
                <p class="font-bold text-3xl mb-1" style="color:#6C63FF;">{{ $categories->count() }}</p>
                <p class="text-sm font-medium" style="color:#6B7280;">Categories</p>
            </div>
            <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm text-center">
                <p class="font-bold text-3xl mb-1" style="color:#6C63FF;">15k+</p>
                <p class="text-sm font-medium" style="color:#6B7280;">Newsletter readers</p>
            </div>
